      ...
      <Container>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
          {displayBenefits.map((benefit, index) => (
            <Card
              key={index}
              className="h-full border-none shadow-sm transition-shadow duration-300 hover:shadow-md"
            >
              <CardContent className="p-6">
                <div className="flex flex-col items-center text-center">
                  <div className="mb-4 rounded-full bg-accent/10 p-4">{benefit.icon}</div>
                  <h3 className="mb-2 text-lg font-semibold text-primary">{benefit.title}</h3>
