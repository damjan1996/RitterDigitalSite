              ...
              >
                  {displayServices.map((service, index) => (
                    <motion.div key={service.id} variants={itemVariants}>
                        <Card
                          className="overflow-hidden h-full border border-[#F4F5F8] shadow-sm hover:shadow-lg transition-all duration-500"
                        >
                            <motion.div
                            ...
                            >
                                {service.image ? (
                                  ...
                                  <div className="w-full h-full bg-[#F4F5F8] flex items-center justify-center">
                                      {service.icon}
                                  </div>
                                )}
                                {/* Subtiler Overlay-Effekt */}
                                <div className="absolute inset-0 bg-[#23282D] bg-opacity-0 hover:bg-opacity-5 transition-all duration-300" />

                                {/* Subtiler Farbakzent in der Ecke */}
                                <div className={`absolute top-0 right-0 w-12 h-1 ${index % 2 === 0 ? 'bg-[#FF8A4C]' : 'bg-[#3A4F66]'}`}></div>
                            </motion.div>

                            <CardContent className="p-6">
                                <div className="flex items-start gap-4">
                                    <motion.div
                                      className={`rounded-full ${index % 2 === 0 ? 'bg-[#FF8A4C]/10' : 'bg-[#3A4F66]/10'} p-3 mt-1`}
                                      whileHover={{
                                          scale: 1.1,
                                    ...
                                    <div>
                                        <motion.h3
                                          className="text-xl font-bold text-[#23282D] mb-2"
                                          whileHover={{
                                              x: 3,
                                        ...
                                        <Link href={service.slug} className="inline-block">
                                            ...
                                            >
                                                <Button
                                                  variant="ghost"
                                                  className={`p-0 h-auto ${index % 2 === 0 ? 'text-[#FF8A4C]' : 'text-[#3A4F66]'} hover:text-[#23282D]`}
                                                >
