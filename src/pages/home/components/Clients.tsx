...
export const Clients: React.FC<ClientsProps> = ({
  ...
  return (
    <section className={cn('relative overflow-hidden bg-[#23282D] py-16', className)}>
      {/* Subtle accent corner decoration */}
      <div className="absolute right-0 top-0 h-40 w-40 rounded-bl-full bg-[#FF8A4C]/5" />
      <div className="absolute bottom-0 left-0 h-40 w-40 rounded-tr-full bg-[#3A4F66]/5" />

      {/* Subtle accent line at the top */}
      <div className="absolute left-0 top-0 h-1 w-full bg-[#FF8A4C]" />
        ...
        >
          {clientsToDisplay.map((client, index) => (
            <motion.div
              ...
              className="flex items-center justify-center rounded-none border border-white/10 bg-white/5 p-4 transition-all duration-300 hover:bg-white/10"
              whileHover={{
                y: -5,
            ...
            >
              <div className="relative flex h-12 w-full items-center justify-center">
                ...
                <span className="text-sm font-medium text-white/70">{client.name}</span>
              </div>
            </motion.div>
          ))}
        </motion.div>
            ...
            {[
              ...
              >
                <div className="mb-2 text-4xl font-bold text-white">
                  {stat.value}
                  <span className="text-[#FF8A4C]">{stat.suffix}</span>
                </div>
