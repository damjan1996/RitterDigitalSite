...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (category) {
        // Sicherstellen, dass wir mit einem einzelnen String arbeiten
        const categoryValue = Array.isArray(category) ? category[0] : category;

        // Überprüfen, ob nach Slug oder ID gefiltert wird
        if (isNaN(parseInt(categoryValue, 10))) {
          // Nach Slug filtern
          query = query.eq('categories.slug', categoryValue);
        } else {
          // Nach ID filtern
          query = query.eq('category_id', parseInt(categoryValue, 10));
        }
      }
      ...
      if (author) {
        ...
        query = query.eq('author_id', authorValue);
      }
