 ...
 */
export const useScroll = (options: UseScrollOptions = {}): ScrollInfo & {
    ...
    const updateScrollInfo = useCallback(() => {
        ...
        if (
            ...
            const newDirection: ScrollDirection = {
                vertical: currentPosition.y > prevPosition.current.y ? 'down' :
                    currentPosition.y < prevPosition.current.y ? 'up' :
                        direction.vertical,
                horizontal: currentPosition.x > prevPosition.current.x ? 'right' :
                    currentPosition.x < prevPosition.current.x ? 'left' :
    ...
    const scrollTo = useCallback((target: ScrollPosition | HTMLElement | string, smooth: boolean = true) => {
        if (typeof window === 'undefined') return;
